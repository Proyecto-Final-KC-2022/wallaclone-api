...
//metodo estático
userSchema.statics.hashPassword = function(clearPassword: string): Promise<string> {
    return bcrypt.hash(clearPassword, 7);
};
//método de instancia
userSchema.methods.comparePassword = function(clearPassword:string):Promise<boolean>  {
...
//   const usrId = new Types.ObjectId(userId);
  const query = User.findByIdAndUpdate(advertId, {
